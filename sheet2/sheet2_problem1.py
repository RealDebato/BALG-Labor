...
S_morph = img - (0.5 * morph_laplace).astype(np.uint8)

# Tophat
WTH = cv2.morphologyEx(img, cv2.MORPH_TOPHAT, kernel)
BTH = cv2.morphologyEx(img, cv2.MORPH_BLACKHAT, kernel)
S_tophat = img - (1 * WTH).astype(np.uint8) + (0.5 * BTH).astype(np.uint8)
...
S_morph_smooth = img - (0.5 * morph_laplace_S).astype(np.uint8)
...
S_tophat_smooth = smooth_gaussian - (1 * WTH).astype(np.uint8) + (0.5 * BTH).astype(np.uint8)
