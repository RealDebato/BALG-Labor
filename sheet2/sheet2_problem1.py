...
S_lin = img - (c * lin_laplace).astype(np.uint8)
...
S_morph = img - (c * morph_laplace).astype(np.uint8)

# Tophat
WTH = cv2.morphologyEx(img, cv2.MORPH_TOPHAT, kernel)
BTH = cv2.morphologyEx(img, cv2.MORPH_BLACKHAT, kernel)
S_tophat = img - (c * WTH).astype(np.uint8) + (c * BTH).astype(np.uint8)
...
S_lin_smooth = smooth_gaussian - (c_smooth * lin_laplace_S).astype(np.uint8)

# Morphological Laplacian --->  0.5 * (Dilatation + Erosion) - img
morph_laplace_S = 0.5 * (cv2.dilate(smooth_gaussian, kernel, iterations=1) + cv2.erode(smooth_gaussian, kernel, iterations=1))
S_morph_smooth = img - (c_smooth * morph_laplace_S).astype(np.uint8)
...
S_tophat_smooth = smooth_gaussian - (c_smooth * WTH).astype(np.uint8) + (c_smooth * BTH).astype(np.uint8)
