...
if __name__ == '__main__':
    ...
    pixel_data_train, labels_data_train, pixel_data_val, labels_data_val, pixel_data_test, labels_data_test = unpickle_cifar(12800, 2000, 10000)
    ...
    hidden_layers = [100, 50, 30, 20]
    learning_rate = 1e-2
    model = MultiLayerFC(340, hidden_layers, 10)
    model.layers.apply(init)
    optimizer = optim.SGD(model.parameters(), lr=learning_rate)

    train(model, optimizer, epochs = 50, lr=learning_rate)

    
    fig, ax1 = plt.subplots()

    fig.suptitle(f'4 Layer - ReLU - lr = 1e-2')
